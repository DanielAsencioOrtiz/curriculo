...
class ArticulacionController extends Controller
{
    ...
    public function storeArticulacion(Request $request){
       ...
       $cantidad_curso =  $cant_electivo + $cant_obligatoria;
           

        $creditos_generales = CursosPlanEstudios::where('ciclo',$request->curso['ciclo'])->where('id_plan_estudio', $request->plan_estudio_id)->where('tipo', 'GENERAL')->where('estado','obligatoria')->sum('creditos'); //AQUI PUEDE VER ALGUNA OBSERVACION CON EL ELECTIVO DE TIPO GENERAL, SERIA A LO ANTERIOR AGREGAR ->where('')
        ...
        //var_dump('Creditos generales: '.$cantidad_creditaje);
        ...
        $creditaje_cursos_generales = CursosPlanEstudios::where('id_plan_estudio', $request->plan_estudio_id)->where('tipo', 'GENERAL')->where('estado','obligatoria')->sum('creditos');
        $creditaje_cursos_generales_electivo = CursosPlanEstudios::where('id_plan_estudio', $request->plan_estudio_id)->where('tipo', 'GENERAL')->where('estado','electivo')->sum('creditos');
        ...
        if(isset($buscar_curso)){
            ...
            if ($programa_estudio->num_ciclos == 10) {
                if($cantidad_curso>=7 || $request->curso['creditos']+$cantidad_creditaje-$credito_electivo>22 || $cantidad_creditaje_total>220){
                    //dd($cantidad_curso);
                   ...
                   $c_general = count(CursosPlanEstudios::where('id_plan_estudio', $request->plan_estudio_id)->where('ciclo', $request->curso['ciclo'])->where('tipo', 'GENERAL')->get());
            ...
            if ($programa_estudio->num_ciclos == 12) {
                if($cantidad_curso>=7 || $request->curso['creditos']+$cantidad_creditaje-$credito_electivo>22 || $cantidad_creditaje_total>264){
                    return response()->json("2");
                }else{
                    $curso = new CursosPlanEstudios();
                    $curso->id_plan_estudio = $request->plan_estudio_id;
            ...
            if ($programa_estudio->num_ciclos == 14) {
                if($cantidad_curso>=7 || $request->curso['creditos']+$cantidad_creditaje-$credito_electivo>22 || $cantidad_creditaje_total>308){
                    return response()->json("2");
                }else{
                    $curso = new CursosPlanEstudios();
                    $curso->id_plan_estudio = $request->plan_estudio_id;
